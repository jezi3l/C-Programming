...
void addRandomMine(board_t * b) {
  int x;
  int y;
  //if board too small for the number
  //of mines that requested, try w*h*10 times before
  //we give up
...
/*
 makeBoard: this function should malloc and initialize a board_t
   ...
   It will call malloc (multiple times)
   ...
   Note that the fields of board_t must be initailzed before
   calling addRandomMine.
...
board_t * makeBoard(int w, int h, int numMines)
{
  ...
  b->board = (int**)malloc(h * sizeof(*b->board));
...
- countMines: this funciton takes a board_t, and an (x,y) coordinate.
   ...
   KNOWN_MINE.  Use the IS_MINE macro to test both cases:
        IS_MINE(b->board[ny][nx]) 
   (where b is the board_t, and (nx,ny) are the coordinates we 
    want to check).  It also checks it doesn't go out of bounds of the array.
*/
...
/*
- checkWin: this funciton takes a board_t and checks if the game has
